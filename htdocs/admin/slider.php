...
if ( $pagePreviewId > "" ) {
	...
	echo "<style type='text/css'>html, body {margin-top:0 !important;} nav, .row, .version {display:none !important;} #wrapper {padding-left: 0 !important;} #page-wrapper {min-height: 200px !important;}</style>";
...
if ( $newSlide == 'true' || $editSlide ) {
	...
	$slide_content = isset( $_POST['slide_content'] ) ? sqlEscapeStr($_POST['slide_content']) : null;
	$start_date    = dateTimeFormat( 1, safeCleanStr( $_POST['start_date'] ) );
	$end_date      = dateTimeFormat( 1, safeCleanStr( $_POST['end_date'] ) );
	$slide_link    = isset( $_POST['slide_link'] ) ? sqlEscapeStr($_POST['slide_link']) : null;
	...
	if ( $editSlide ) {
		...
		if ( ! empty( $_POST ) ) {
			...
			$slideUpdate = "UPDATE slider SET title='" . $slide_title . "', content='" . $slide_content . "', startdate='" . $start_date . "', enddate='" . $end_date . "', link='" . $slide_link . "', image='" . $slide_image . "', loc_type='" . $location_type . "', author_name='" . $_SESSION['user_name'] . "' WHERE id=" . $editSlide . " AND guid='" . $theslideGuid . "' AND loc_id=" . loc_id . ";";
		...
		$sqlSlides = mysqli_query( $db_conn, "SELECT id, title, image, content, guid, startdate, enddate, link, loc_type, active, sort, author_name, datetime, loc_id FROM slider WHERE id=" . $editSlide . " AND guid='" . $theslideGuid . "' AND loc_id=" . loc_id . ";" );
	...
	if ( isset( $_POST['save_main'] ) ) {
		...
		for ( $i = 0; $i < $slide_count; $i ++ ) {
			...
			$slide_sort      = isset( $_POST['slide_sort'][ $i ] ) ? safeCleanStr( addslashes( $_POST['slide_sort'][ $i ] ) ) : null;
			...
			$slide_id        = isset( $_POST['slide_id'][ $i ] ) ? safeCleanStr( $_POST['slide_id'][ $i ] ) : null;
