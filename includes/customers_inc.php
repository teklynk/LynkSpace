...
if ($customerNumRows > 0) {
    ...
    while ($rowCatCustomers = mysqli_fetch_array($sqlCatCustomers)) {
        ...
        $sqlCustomers = mysqli_query($db_conn, "SELECT id, image, icon, name, link, catid, section, content, featured, datetime, active, loc_id FROM customers WHERE active='true' AND section='" . $customerSection . "' AND featured='false' AND catid=".$customerCatId." AND loc_id=".$custDefaultLoc." ORDER BY name ASC"); //While loop
