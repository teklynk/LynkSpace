	...
	if (!empty($_POST)) {
		if (!empty($_POST['about_heading'])) {
			...
			if ($rowAbout['loc_id'] == $_GET['loc_id']) {
				//Do Update
				$aboutUpdate = "UPDATE aboutus SET heading='".safeCleanStr($_POST['about_heading'])."', content='".$_POST['about_content']."', image='".$_POST['about_image']."', image_align='".$_POST['about_image_align']."', datetime='".date("Y-m-d H:i:s")."' WHERE loc_id=".$_GET['loc_id']." ";
				mysqli_query($db_conn, $aboutUpdate);
			} else {
				//Do Insert
				$aboutInsert = "INSERT INTO aboutus (heading, content, image, image_align, datetime, loc_id) VALUES ('".safeCleanStr($_POST['about_heading'])."', '".$_POST['about_content']."', '".$_POST['about_image']."', '".$_POST['about_image_align']."', '".date("Y-m-d H:i:s")."', ".$_GET['loc_id'].")";
