...
$sqlCustomers = mysqli_query($db_conn, "SELECT id, section, datetime, cust_loc_id FROM category_customers ORDER BY datetime DESC");
while ($rowCustomers  = mysqli_fetch_array($sqlCustomers)) {
    ...
    $custDate=$rowCustomers['datetime'];

        $writeline = "\t<url>\n";
        fwrite($sitemapfile, $writeline);
        $writeline = "\t\t<loc>" . str_replace('admin/sitemapbuilder.php/', '', htmlspecialchars("http://" . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . "/databases.php?section=" . $custSection . "&cat_id=" . $custCatId . "&loc_id=" . $custLocId)) . "</loc>\n";
        fwrite($sitemapfile, $writeline);
        $writeline = "\t\t<lastmod>" . $pageDate . "</lastmod>\n";
        fwrite($sitemapfile, $writeline);
        $writeline = "\t\t<changefreq>monthly</changefreq>\n";
