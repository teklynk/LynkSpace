...
if (!empty($_POST)) {
    if ($_POST['not_robot'] == 'e6a52c828d56b46129fbf85c4cd164b3') {
        ...
        $userLogin = mysqli_query($db_conn, "SELECT id, username, password, email, level, loc_id FROM users WHERE username='".strip_tags(trim($_POST['username']))."' AND password=password('".strip_tags(trim($_POST['password']))."') AND email='".filter_var(trim($_POST['email']), FILTER_VALIDATE_EMAIL)."' LIMIT 1");
