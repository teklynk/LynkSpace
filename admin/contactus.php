...
include 'includes/header.php';
	...
	if (!empty($_POST['contact_heading'])) {
		...
		if($rowContact['loc_id'] == $_GET['loc_id']){
			//Do Update
			$contactUpdate = "UPDATE contactus SET heading='".htmlspecialchars(strip_tags(trim($_POST['contact_heading'])), ENT_QUOTES)."', introtext='".htmlspecialchars(strip_tags(trim($_POST['contact_introtext'])), ENT_QUOTES)."', mapcode='".mysqli_real_escape_string($db_conn, $_POST['contact_mapcode'])."', email='".filter_var($_POST['contact_email'], FILTER_VALIDATE_EMAIL)."', sendtoemail='".filter_var($_POST['contact_sendtoemail'], FILTER_VALIDATE_EMAIL)."', address='".htmlspecialchars(strip_tags(trim($_POST['contact_address'])), ENT_QUOTES)."', city='".htmlspecialchars(strip_tags(trim($_POST['contact_city'])), ENT_QUOTES)."', state='".htmlspecialchars(strip_tags(trim($_POST['contact_state'])), ENT_QUOTES)."', zipcode='".htmlspecialchars(strip_tags(trim($_POST['contact_zipcode'])), ENT_QUOTES)."', phone='".htmlspecialchars(strip_tags(trim($_POST['contact_phone'])), ENT_QUOTES)."', hours='".htmlspecialchars(strip_tags($_POST['contact_hours']), ENT_QUOTES)."', datetime='".date("Y-m-d H:i:s")."' WHERE loc_id=".$_GET['loc_id']." ";
			mysqli_query($db_conn, $contactUpdate);
		} else {
			//Do Insert
			$contactInsert = "INSERT INTO contactus (heading, introtext, mapcode, email, sendtoemail, address, city, state, zipcode, phone, hours, datetime, loc_id) VALUES ('".htmlspecialchars(strip_tags(trim($_POST['contact_heading'])), ENT_QUOTES)."', '".htmlspecialchars(strip_tags(trim($_POST['contact_introtext'])), ENT_QUOTES)."', '".mysqli_real_escape_string($db_conn, $_POST['contact_mapcode'])."', '".filter_var($_POST["contact_email"], FILTER_VALIDATE_EMAIL)."', '".filter_var($_POST['contact_sendtoemail'], FILTER_VALIDATE_EMAIL)."', '".htmlspecialchars(strip_tags(trim($_POST['contact_address'])), ENT_QUOTES)."', '".htmlspecialchars(strip_tags(trim($_POST['contact_city'])), ENT_QUOTES)."', '".htmlspecialchars(strip_tags(trim($_POST['contact_state'])), ENT_QUOTES)."', '".htmlspecialchars(strip_tags(trim($_POST['contact_zipcode'])), ENT_QUOTES)."', '".htmlspecialchars(strip_tags($_POST['contact_phone']), ENT_QUOTES)."', '".htmlspecialchars(strip_tags($_POST['contact_hours']), ENT_QUOTES)."', '".date("Y-m-d H:i:s")."', ".$_GET['loc_id'].")";
