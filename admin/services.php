...
if ($_GET['preview']>"") {
	...
	$sqlServicesPreview = mysqli_query($db_conn, "SELECT id, title, icon, image, content, link FROM services WHERE id='$pagePreviewId' AND loc_id=".$_SESSION['loc_id']." ");
	...
	echo "<style type='text/css'>html, body {margin-top:0px !important;} nav, .row, .version {display:none !important;} #wrapper {padding-left: 0px !important;}</style>";
		...
		echo $rowServicesPreview['content'];

		if ($rowServicesPreview['link']>0){
			echo "<br/><p><i class='fa fa-fw fa-external-link'></i> <a href='../page.php?page_id=".$rowServicesPreview['link']."' target='_blank'>Page Link</a></p>";
		}
...
<?php
	...
	if ($_GET['newservice'] OR $_GET['editservice']) {
		...
		if ($_GET['editservice']) {
			...
			$sqlServices = mysqli_query($db_conn, "SELECT id, title, icon, image, content, link, active, datetime FROM services WHERE id='$theserviceId' AND loc_id=".$_GET['loc_id']." ");
		...
		//Create new service
			...
			if (!empty($_POST['service_title'])) {
				$servicesInsert = "INSERT INTO services (title, content, icon, image, link, active, loc_id) VALUES ('".$_POST['service_title']."', '".$_POST['service_content']."', '".$_POST['service_icon_select']."', '".$_POST['service_image_select']."', ".$_POST['service_link'].", 'true', ".$_GET['loc_id'].")";
...
?>
	...
	<form name="serviceForm" method="post">
		<div class="row">
			<div class="col-lg-4">
				<div class="form-group" id="serviceactive">
					<label>Active</label>
					<div class="checkbox">
						<label>
		...
		<div class="form-group">
			...
			<input class="form-control input-sm" name="service_title" value="<?php if($_GET['editservice']){echo $rowServices['title'];} ?>" placeholder="Service Title">
		</div>
		<hr/>
		<div class="form-group">
		...
		<?php
	        if ($rowServices['image']=="") {
				...
				$imgSrc = "../uploads/".$rowServices['image'];
			}
		...
		<div class="form-group">
			...
			<select class="form-control input-sm" name="service_icon_select" id="service_icon_select">
				...
				<?php
					...
					while ($rowServicesIcon = mysqli_fetch_array($sqlServicesIcon)) {

						$icon=$rowServicesIcon['icon'];
...
<?php
    ...
    if (($_POST['main_heading'])) {
        $setupUpdate = "UPDATE setup SET servicesheading='".$_POST['main_heading']."', servicescontent='".$_POST['main_content']."' WHERE loc_id=".$_GET['loc_id']." ";
			...
			<table class="table table-bordered table-hover table-striped">
        		...
        		<?php
					...
					while ($rowServices = mysqli_fetch_array($sqlServices)) {
						...
						echo "<tr>
						<td><a href='?loc_id=".$_GET['loc_id']."&editservice=$serviceId' title='Edit'>".$serviceTitle."</a></td>
						<td class='col-xs-1'>
						...
						<button type='button' data-toggle='tooltip' title='Move' class='btn btn-xs btn-default' onclick=\"window.location.href='?loc_id=".$_GET['loc_id']."&moveservice=$serviceId&movetitle=$serviceTitle'\"><i class='fa fa-fw fa-arrow-up'></i></button>
						<button type='button' data-toggle='tooltip' title='Delete' class='btn btn-xs btn-default' onclick=\"window.location.href='?loc_id=".$_GET['loc_id']."&deleteservice=$serviceId&deletetitle=$serviceTitle'\"><i class='fa fa-fw fa-trash'></i></button>
						</td>
						</tr>";
					}
				?>
