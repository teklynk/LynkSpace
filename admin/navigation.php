	...
	if (!empty($_POST)) {
		...
		if (!empty($_POST['nav_newname'])) {
			...
			if (!empty($_POST['nav_newcat']) AND $_POST['exist_cat']=="") {
				$navNewCat = "INSERT INTO category (name, datetime, nav_loc_id) VALUES ('".$_POST['nav_newcat']."', '".date("Y-m-d H:i:s")."', ".$_SESSION['loc_id'].")";
			...
			$navNew = "INSERT INTO navigation (name, url, sort, catid, section, win, datetime, loc_id) VALUES ('".$_POST['nav_newname']."', '".$_POST['nav_newurl']."', 0, $getTheCat, '".$getNavSection."','off', '".date("Y-m-d H:i:s")."', ".$_SESSION['loc_id'].")";
		...
		for($i=0; $i<$_POST['nav_count']; $i++) {
			...
			$navUpdate = "UPDATE navigation SET sort=".$_POST['nav_sort'][$i].", name='".$_POST['nav_name'][$i]."', url='".$_POST['nav_url'][$i]."', catid=".$_POST['nav_cat'][$i].", datetime='".date("Y-m-d H:i:s")."', loc_id=".$_GET['loc_id']." WHERE id=".$_POST['nav_id'][$i]." ";
...
?>
	...
	<div class="row">
		...
		if ($_GET['renamecat'] AND $_GET['newcatname'] AND !$_GET['confirm']) {
			...
			$navRenameCatUpdate = "UPDATE category SET name='".$renameCatTitle."', datetime='".date("Y-m-d H:i:s")."' WHERE id='$renameCatId'";
		...
		// add category
		if ($_GET['addcatname'] > "") {
            $navAddCat = "INSERT INTO category (name, datetime, nav_loc_id) VALUES ('".$_GET['addcatname']."', '".date("Y-m-d H:i:s")."', ".$_SESSION['loc_id'].")";
