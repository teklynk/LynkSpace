	...
	if (!empty($_POST)) {
		if (!empty($_POST['generalinfo_heading'])) {
			...
			if ($rowGeneralinfo['loc_id'] == $_GET['loc_id']) {
				//Do Update
				$generalinfoUpdate = "UPDATE generalinfo SET heading='".htmlspecialchars(strip_tags(trim($_POST["generalinfo_heading"])), ENT_QUOTES)."', content='".trim($_POST["generalinfo_content"])."', datetime='".date("Y-m-d H:i:s")."' WHERE loc_id=".$_GET['loc_id']." ";
				mysqli_query($db_conn, $generalinfoUpdate);
			} else {
				//Do Insert
				$generalinfoInsert = "INSERT INTO generalinfo (heading, content, datetime, loc_id) VALUES ('".htmlspecialchars(strip_tags(trim($_POST['generalinfo_heading'])), ENT_QUOTES)."', '".trim($_POST['generalinfo_content'])."', '".date("Y-m-d H:i:s")."', ".$_GET['loc_id'].")";
